...
def main():
    ...
    parser.add_argument("--use-pan-lima-score", action="store_true", default=False,
                        help="use LIMA correlation statistic from .score file in mosaic score (generated by pgc_ortho)")
    parser.add_argument("--lima-score-only", action="store_true", default=False,
                        help="use only LIMA score for weighting (warning: overrides other scoring metrics) (must set --use-pan-lima-score first)")
    ...
    #### Validate LIMA score options
    if args.lima_score_only and not args.use_pan_lima_score:
        parser.error("Option --lima-score-only can only be used when --use-pan-lima-score is specified.")
        sys.exit(1)
...
def build_shp(contribs, shp, args, params):
    ...
    for iinfo,geom in contribs:
        ...
        feat.SetField("LIMA_SCORE", iinfo.lima_score)
